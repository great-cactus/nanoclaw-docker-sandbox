...
Akira の Obsidian vault は **このコンテナ内に直接マウントされている**：

- **vault root = `/workspace/extra/obsidian`**（読み書き可能）
- このスキル内で `GTD/Tasks/...`、`Dashboard.md` などと書くパスは、すべて vault root からの相対パス。実際の絶対パスは `/workspace/extra/obsidian/` を前に付けたもの（例：`/workspace/extra/obsidian/GTD/Tasks/<title>.md`）。

操作は **通常のファイルツール（Read / Write / Edit / Glob / Grep）で vault のファイルを直接読み書きする**。`obsidian` という CLI コマンドや、IPC／MCP 経由の `obsidian` ツールは **存在しない。使ってはいけない**（呼ぶと応答が返らずタイムアウトする）。Obsidian アプリを起動する必要もない。
