...
**Location**: `~/.claude/skills/deep-research/tools/paper_finder/paper_finder.py`
...
```bash
python ~/.claude/skills/deep-research/tools/paper_finder/paper_finder.py --mode scrape --config <config.yaml>
python ~/.claude/skills/deep-research/tools/paper_finder/paper_finder.py --mode download --jsonl <results.jsonl>
python ~/.claude/skills/deep-research/tools/paper_finder/paper_finder.py --list-venues
```
...
Supports `--peer-reviewed-only` and `--top-conferences` filters. API key: stored in `/workspace/group/research-api-keys.env` (field `S2_API_KEY`). Load with: `$(grep S2_API_KEY /workspace/group/research-api-keys.env 2>/dev/null | cut -d= -f2)`
